...
void initPlateau(char plateau[15][15]) { //
	...
	plateau[7][7]='X';// centre du plateau (case 80 en coordonnées 1D)
	for(i=1;i<7;i++){
        plateau[7][i]= '0' + i;
    }//escalier joueur 1
	for(i=1;i<7;i++){
...
void afficherPlateau(char plateau[15][15], int positionDesPions[], int nombreDejoueurs) {
    ...
    for (int i = 0; i < nombreDejoueurs; i++) {
        //on recupere chaque coordonnée de pion en une dimension et on les traduits une par une en coordonnées a deux dimensions
...
/*
  ...
  retour : cette fonction va traduire la coordonnée en une dimension en coordonnées a deux dimensions et la retourner

  exemple, la case centrale est située à la coordonnée 80 en une dimension
...
/*
  param1  numéro du joueur
  param2 : coordonnée  actuelle du joueur en une dimension sur le plateau (de 0 à 80)
  param3: la valeur du dé qu'il vient de jouer

  retour : cette fonction retourne la nouvelle coordonnée
...
/*
  param1  positionCouranteDuJoueur (coordonnée en 1 dimension)

...
int main(int argc, char **argv) {
    ...
    int numFils;//pour stocker les numéros des processus fils
    pid_t pid;
    ...
    numProchain = readPremierJoueur(nombreDejoueurs);//demande à l'utilisateur d'entrer le numéro du prochain joueur
    ...
    if (pid == 0) {//
             //declarations des variables des fils ici
            ...
            while (!jeuTermine) { //les fils bouclent tant que le jeu n'est pas terminé
                ...
                if (numFils == quiJoue) { //si mon numéro de processus vaut celui qui doit jouer => je joue
                    ...
                    //on met à jour le plateau à la case correspondant à la nouvelle position
                    ...
                    if (infosTour.newPosition == 80) //si celui qui a joué est à la position 80 on arrête le jeu
                        jeuTermine = true;
                    ...
                    if (numFils == numDernier(quiJoue, nombreDejoueurs)) { //processus qui envoie le code de retour au pere
                        ...
                        //informations à envoyer au processus pere
